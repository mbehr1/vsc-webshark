...
import {
  ...
  WebsharkView,
  SharkdProcess,
  WebsharkViewSerializer,
  SelectedTimeData,
  WebsharkViewReadonlyEditorProvider,
} from './websharkView';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (extension) {
    ...
    reporter = new TelemetryReporter(`InstrumentationKey=${strK}`);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('webshark.filterPcap', async () => {
      return vscode.window
        .showOpenDialog({
          canSelectFiles: true,
          canSelectFolders: false,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('webshark.extractDlt', async () => {
      return vscode.window
        .showOpenDialog({
          canSelectFiles: true,
          canSelectFolders: false,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('webshark.removeTecmp', async () => {
      return vscode.window
        .showOpenDialog({
          canSelectFiles: true,
          canSelectFolders: false,
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer(
      'vsc-webshark',
      new WebsharkViewSerializer(
        reporter,
        treeDataProvider,
        _onDidChangeSelectedTime,
        <string>vscode.workspace.getConfiguration().get('vsc-webshark.sharkdFullPath'),
        context,
        activeViews,
        (r) => {
          const idx = activeViews.indexOf(r);
